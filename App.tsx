...
const SidebarLink = ({
  ...
  return (
    ...
    >
      <i
        className={`fas ${icon} w-6 text-center text-lg ${active ? "text-white" : "text-slate-400 group-hover:text-[#14452F]"} transition-colors`}
      ></i>
      <span className="tracking-wide font-[Roboto]">{label}</span>
      {active && (
        <div className="ml-auto w-1.5 h-1.5 bg-white chamfer-sm"></div>
      )}
    </Link>
  );
};
...
const ProtectedRoute = ({
  ...
  if (loading)
    return (
      <div className="h-full w-full flex items-center justify-center font-[Roboto] text-sm text-[#14452F] font-bold uppercase tracking-widest">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <div>
              <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter uppercase leading-tight">
                Xin chào, <br />{" "}
                <span className="text-[#14452F]">{user?.fullName}</span>
              </h1>
              <p className="text-slate-500 text-sm font-medium max-w-xl leading-relaxed mt-4">
                Chào mừng trở lại trung tâm điều hành. Hệ thống AI đang ở trạng
                thái sẵn sàng hỗ trợ công việc giảng dạy và học tập của bạn.
              </p>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 bg-[#14452F] text-white chamfer-xl chamfer-shadow p-10 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="flex justify-between items-start z-10">
            ...
            <div className="text-right">
              ...
              <div className="text-[10px] font-bold uppercase tracking-[0.3em] text-green-400 mt-1">
                Realtime Clock
              </div>
            </div>
          </div>
...
const StatCard = ({ icon, label, value, unit, color }: any) => {
  ...
  return (
    <div className="bg-white p-6 chamfer-lg chamfer-shadow border border-slate-100 flex flex-col justify-between h-40 hover:-translate-y-1 transition-all duration-300 group relative overflow-hidden">
      <div
        className={`absolute top-0 right-0 w-16 h-16 chamfer-sm -mr-6 -mt-6 opacity-20 ${activeColor.split(" ")[1]}`}
      ></div>

      <div className="flex justify-between items-start relative z-10">
        <div
          className={`w-12 h-12 flex items-center justify-center chamfer-sm text-xl ${activeColor}`}
        >
      ...
      <div className="mt-auto relative z-10">
        ...
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1 flex items-center gap-2">
          {label}{" "}
          <span className="bg-slate-100 text-slate-500 px-1.5 py-0.5 chamfer-sm">
            {unit}
          </span>
        </p>
      </div>
...
const AppContent: React.FC = () => {
  ...
  if (authLoading || !isDataLoaded)
    return (
      <div className="h-screen w-screen flex flex-col items-center justify-center bg-white text-[#14452F] font-[Roboto]">
    ...
    <div className="flex h-screen bg-[#F0F2F5] font-[Roboto] overflow-hidden relative">
              ...
              `}
                >
                  <div className="p-8 border-b border-slate-100 flex items-center gap-4 bg-white">
                    ...
                    <div>
                      <h2 className="text-lg font-black text-[#14452F] tracking-tight uppercase">
                        ĐTS LMS
                      </h2>
                  ...
                  <nav className="flex-1 py-6 overflow-y-auto custom-scrollbar space-y-1">
                    {user?.role === "admin" && (
                      <>
                        <div className="px-8 mb-3 mt-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                          Quản trị Hệ thống
                        </div>
                        <SidebarLink
                          to="/admin"
                    ...
                    {user?.role === "teacher" && (
                      <>
                        <div className="px-8 mb-3 mt-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                          Khu vực Giảng viên
                        </div>
                        <SidebarLink
                          to="/teacher/students"
                  ...
                  <div className="p-6 border-t border-slate-100 bg-slate-50">
                    <Link
                      to="/profile"
                      className="flex items-center gap-4 mb-4 bg-white p-3 chamfer-sm border border-slate-200 hover:border-[#14452F] transition-all cursor-pointer group"
                    >
                      ...
                      <div className="flex flex-col overflow-hidden">
                        <span className="text-sm font-bold text-slate-800 truncate group-hover:text-[#14452F] transition-colors">
                          {user?.fullName}
                        </span>
                        <span className="text-[9px] text-[#14452F] font-bold uppercase tracking-wider">
                          {user?.role === "admin"
                            ? "Quản trị viên"
                            : user?.role === "teacher"
                              ? "Giảng viên"
                    ...
                    <button
                      onClick={signOut}
                      className="w-full py-3 bg-white border border-slate-200 text-slate-600 chamfer-sm text-[10px] font-black uppercase tracking-widest hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-all flex items-center justify-center gap-2"
                    >
                  ...
                  <div className="flex-1 overflow-auto custom-scrollbar p-0">
                    <Routes>
                      ...
                      <Route
                        ...
                        element={
                          <Documents
                            onUpdateKnowledgeBase={(chunks) =>
                              setKnowledgeBase((p) => [...p, ...chunks])
                            }
                            onDeleteDocumentData={(id) =>
                              setKnowledgeBase((p) =>
                                p.filter((c) => c.docId !== id),
                              )
                            }
                            onNotify={showNotify}
                          />
      ...
      <div className="fixed top-6 right-6 z-[100] space-y-3 pointer-events-none">
        {notifications.map((n) => (
          <div
            key={n.id}
            className={`px-6 py-4 bg-white border-l-4 chamfer-shadow chamfer-md flex items-center gap-4 animate-slide-up pointer-events-auto min-w-[340px] transform hover:scale-105 transition-all ${
              n.type === "success"
                ? "border-[#14452F]"
                : n.type === "error"
                  ? "border-red-500"
          ...
          >
            <div
              className={`w-8 h-8 chamfer-sm flex items-center justify-center shrink-0 ${
                n.type === "success"
                  ? "bg-[#E8F5E9] text-[#14452F]"
                  : n.type === "error"
